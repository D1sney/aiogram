...
async def read_command(message: types.Message):
    pictures = []
    teksts = []
    file = open( r'stories.txt', 'r', encoding='utf8')
    ...
    idishnik = await bot.send_photo(chat_id=message.chat.id,                    # idishnik берет информацию о сообщени, нам это нужно чтобы узнать точный ID сообщения для будущего удаления его                  
                         ...
                         reply_markup=ikb)
    await message.delete()
    file = open( r'random_story_now.txt', 'r', encoding='utf8')
    flag = False
    id = file.readline()
    while id != '':
        id = int(id)
    ...
    file.close()
    if flag == False:
        file = open( r'random_story_now.txt', 'a', encoding='utf8')
        file.write(f'{message.chat.id}\n{random_photo}\n')
        file.close()
    else:
        file = open( r'random_story_now.txt', 'r', encoding='utf8')
        newfile = open( r'new_random_story_now.txt', 'w', encoding='utf8')
        id = file.readline()
        while id != '':
            id = int(id)
        ...
        file.close()
        newfile.close()
        os.remove(r'random_story_now.txt')
        os.rename(r'new_random_story_now.txt', r'random_story_now.txt')
    
    # все операции с будущим удалением окна "Читать Истории"
    file = open( r'id_story.txt', 'r', encoding='utf8')
    flag = False   
    id = file.readline()
    while id != '':
        id = int(id)
    ...
    file.close()
    if flag == False:
        file = open( r'id_story.txt', 'a', encoding='utf8')
        file.write(f'{message.chat.id}\n{idishnik.message_id}\n')
        file.close()
    if flag == True:
        try:
            ...
            pass
    file = open( r'id_story.txt', 'r', encoding='utf8')
    newfile = open( r'new_id_story.txt', 'w', encoding='utf8')
    id = file.readline()
    while id != '':
        id = int(id)
    ...
    file.close()
    newfile.close()
    os.remove(r'id_story.txt')
    os.rename(r'new_id_story.txt', r'id_story.txt')
...
async def read_love_command(message: types.Message):               
    position = 0
    flag = False
    file = open( r'tochka_chtenia.txt', 'r', encoding='utf8')
    id = file.readline()
    while id != '':
        id = int(id)
    ...
    file.close()
    if flag == False:
        file = open( r'tochka_chtenia.txt', 'a', encoding='utf8')
        file.write(f'{message.chat.id}\n0\n')
        file.close()
    try:
        ...
        if flag2 == True:                                        # флаг показывает есть ли в данный момент истории в избранном или нет
            ...
            flag3 = False
            file = open(r'love_story_now.txt', 'r', encoding='utf8')
            id = file.readline()
            while id != '':
                id = int(id)
            ...
            file.close()
            if flag3 == False:
                file = open(r'love_story_now.txt', 'a', encoding='utf8')
                file.write(f'{message.chat.id}\n{love_pictures[position]}\n')
                file.close()
            else:
                file = open(r'love_story_now.txt', 'r', encoding='utf8')
                newfile = open(r'new_love_story_now.txt', 'w', encoding='utf8')
                id = file.readline()
                while id != '':
                    id = int(id)
                    skip_love_photo = file.readline()
                ...
                file.close()
                newfile.close()
                os.remove(r'love_story_now.txt')
                os.rename(r'new_love_story_now.txt', r'love_story_now.txt')
        else:
            await message.answer('У вас сейчас нету фотографий в избранном')

        # все операции с будущим удалением окна "Читать Истории"
        file = open( r'id_love_story.txt', 'r', encoding='utf8')
        flag = False   
        id = file.readline()
        while id != '':
            id = int(id)
        ...
        file.close()
        if flag == False:
            file = open( r'id_love_story.txt', 'a', encoding='utf8')
            file.write(f'{message.chat.id}\n{idishnik.message_id}\n')
            file.close()
        if flag == True:
            try:
                ...
                pass
        file = open( r'id_love_story.txt', 'r', encoding='utf8')
        newfile = open( r'new_id_love_story.txt', 'w', encoding='utf8')
        id = file.readline()
        while id != '':
            id = int(id)
            nowmessage_id = file.readline()
        ...
        file.close()
        newfile.close()
        os.remove(r'id_love_story.txt')
        os.rename(r'new_id_love_story.txt', r'id_love_story.txt')
    finally:
        await message.delete()                      
...
async def swipe_command(callback: types.CallbackQuery):
    file = open( str(callback.message.chat.id) + '.txt', 'r', encoding='utf8')
    count = 0
    love_pictures = []
    love_teksts = []
    love_picture = file.readline()
    ...
    position = 0
    file = open( r'tochka_chtenia.txt', 'r', encoding='utf8')
    newfile = open( r'new_tochka_chtenia.txt', 'w', encoding='utf8')
    id = file.readline()
    while id != '':
        skip_position = file.readline()
        id = int(id)
    ...
    file.close()
    newfile.close()
    os.remove(r'tochka_chtenia.txt')
    os.rename(r'new_tochka_chtenia.txt', r'tochka_chtenia.txt')
...
async def like_command(callback: types.CallbackQuery):
    ...
    try:     
        ...
        if flag == False:
            if callback.data == 'like_common':
                file = open( str(callback.message.chat.id) + '.txt', 'a', encoding='utf8')
            ...
            if callback.data == 'like_love':
                ...
                file = open( str(callback.message.chat.id) + '.txt', 'r', encoding='utf8')
                newfile = open( r'new_' + str(callback.message.chat.id) + '.txt', 'w', encoding='utf8')
                flag = False
                count = 0
                ...
                file.close()
                newfile.close()                
                os.remove( str(callback.message.chat.id) + '.txt')
                os.rename(r'new_' + str(callback.message.chat.id) + '.txt', str(callback.message.chat.id) + '.txt')
